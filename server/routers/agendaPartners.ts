...
export const agendaRouter = router({
      ...
      try {
        ...
        if (isDuplicateEntryError(error)) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Já existe um evento com esse identificador (slug). Ajuste o título para gerar outro.",
          });
        }
        throw error;
      }
  ...
  update: adminProcedure
    .input(
      culturalEventFields.partial().extend({ id: z.number().int().positive() })
    )
    .mutation(async ({ input }) => {
...
export const partnersRouter = router({
  submit: publicProcedure
    .input(partnerSubmissionFields)
    .mutation(async ({ input, ctx }) => {
      enforceRateLimit(ctx.req, "partners.submit", {
        max: 5,
        windowMs: 10 * 60_000,
      });
  ...
  updateEditorialStatus: adminProcedure
    .input(
      z.object({
