...
const validEvent = {
  ...
  summary:
    "Programação cultural com período, local e referência editorial suficientes para a etapa de curadoria.",
...
describe("camada editorial de agenda e parceiros", () => {
  ...
  it("aceita programação de um dia sem data de término", () => {
    expect(
      culturalEventFields.parse({ ...validEvent, endsAt: null })
    ).toMatchObject({ endsAt: null });
  });

  it("rejeita evento sem URL de fonte", () => {
    expect(() =>
      culturalEventFields.parse({ ...validEvent, sourceUrl: "sem-fonte" })
    ).toThrow();
  });

  it("rejeita fontes com esquemas perigosos em qualquer ambiente", () => {
    expect(() =>
      culturalEventFields.parse({
        ...validEvent,
    ...
    expect(() =>
      culturalEventFields.parse({
        ...validEvent,
        sourceUrl: "data:text/html;base64,PGI+",
      })
    ).toThrow();
  });
