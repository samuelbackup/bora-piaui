...
const eventInput = {
  ...
  summary:
    "Programação cultural confirmada, com período, local e referência registrados para a curadoria editorial.",
...
const partnerInput = {
  ...
  description:
    "Ateliê voltado a peças artesanais autorais que dialogam com técnicas locais e roteiros culturais do território.",
  plan: "destaque" as const,
};

const adminContext = {
  user: { id: 1, openId: "admin_user", name: "Admin", role: "admin" },
} as never;
...
describe("fluxo demonstrativo persistido de agenda e parceiros", () => {
  ...
  it("bloqueia chamadas anônimas em endpoints administrativos", async () => {
    ...
    await expect(caller.create(eventInput)).rejects.toMatchObject({
      code: "FORBIDDEN",
    });
    await expect(
      caller.update({ id: 12, published: true })
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(caller.delete({ id: 12 })).rejects.toMatchObject({
      code: "FORBIDDEN",
    });
    await expect(partnersCaller.adminList()).rejects.toMatchObject({
      code: "FORBIDDEN",
    });
    await expect(
      partnersCaller.updateEditorialStatus({
        id: 44,
  ...
  it("cria evento e persiste sua publicação editorial", async () => {
    ...
    db.updateCulturalEvent.mockResolvedValue({
      ...storedEvent,
      published: true,
    });
    const caller = agendaRouter.createCaller(adminContext);

    await expect(caller.create(eventInput)).resolves.toMatchObject({
      id: 12,
    ...
    expect(db.updateCulturalEvent).toHaveBeenCalledWith(
      12,
  ...
  it("preserva período de vários dias e rejeita término anterior ao início", async () => {
    ...
    await expect(
      caller.create({ ...eventInput, endsAt: "2026-09-14T18:00:00.000Z" })
    ).resolves.toMatchObject({ id: 13 });
    await expect(
      caller.create({ ...eventInput, endsAt: "2026-09-11T18:00:00.000Z" })
  ...
  it("remove evento persistido e informa quando o registro não existe", async () => {
    ...
    await expect(caller.delete({ id: 12 })).resolves.toMatchObject({
      id: 12,
      title: eventInput.title,
    });
  ...
  it("registra proposta pública como pendente e persiste sua revisão", async () => {
    const storedProposal = {
      id: 44,
      ...partnerInput,
      openingHours: partnerInput.openingHours,
      editorialStatus: "pendente" as const,
      editorialNotes: null,
    };
    db.createPartnerSubmission.mockResolvedValue(storedProposal);
    db.getPartnerSubmissionById.mockResolvedValue(storedProposal);
    db.updatePartnerSubmission.mockResolvedValue({
      ...storedProposal,
      editorialStatus: "em_revisao",
    });
    const caller = partnersRouter.createCaller(adminContext);

    await expect(caller.submit(partnerInput)).resolves.toMatchObject({
      id: 44,
      editorialStatus: "pendente",
    });
    await expect(
      caller.updateEditorialStatus({
        id: 44,
        editorialStatus: "em_revisao",
        editorialNotes: null,
      })
    ).resolves.toMatchObject({ editorialStatus: "em_revisao" });
    expect(db.createPartnerSubmission).toHaveBeenCalledWith(
      expect.objectContaining({
        editorialStatus: "pendente",
        editorialNotes: null,
      })
    );
    expect(db.updatePartnerSubmission).toHaveBeenCalledWith(44, {
      editorialStatus: "em_revisao",
      editorialNotes: null,
    });
  });
