...
const validDestination = {
  ...
  summary:
    "Uma síntese editorial suficiente para orientar a descoberta inicial do destino.",
  description:
    "Uma descrição editorial suficientemente longa para representar o destino, suas referências territoriais e os cuidados de planejamento necessários para a visita.",
...
describe("destinationFields", () => {
  it("aceita uma ficha editorial com informações operacionais pendentes", () => {
    expect(destinationFields.parse(validDestination)).toMatchObject(
      validDestination
    );
  });
  ...
  it("rejeita slug com caracteres fora do padrão público", () => {
    expect(() =>
      destinationFields.parse({ ...validDestination, slug: "Rota da Caatinga" })
    ).toThrow();
  });

  it("rejeita URLs operacionais inválidas", () => {
    expect(() =>
      destinationFields.parse({ ...validDestination, routeUrl: "rota-local" })
    ).toThrow();
  });
  ...
  it("rejeita esquemas perigosos nas URLs externas", () => {
    ...
    expect(() =>
      destinationFields.parse({
        ...validDestination,
        routeUrl: "data:text/html;base64,PGI+",
      })
    ).toThrow();
  });
