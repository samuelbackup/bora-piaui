...
describe("seed do catálogo de cidades-piloto", () => {
  it("valida todas as cidades reais contra o schema de inserção", () => {
    ...
    expect(rows.map(row => row.slug)).toEqual(["teresina", "cajueiro-da-praia", "sao-raimundo-nonato"]);
  ...
  it("converte os locais preservando fonte, imagem e status editorial", () => {
    ...
    const theatro = rows.find(row => row.externalId === "theatro-4-de-setembro");
    ...
    expect(theatro?.sourceName).toBe("Mapa da Cultura do Piauí · Theatro 4 de Setembro");
  ...
  it("converte negócios curados, roteiros, tópicos e destaques sem divergência de tipos", () => {
    ...
    expect(buildCuratedBusinessRows(resolveCityId)[0].externalId).toBe("icmbio-atendimento-serra");
  });
  ...
  it("detecta referências quebradas no catálogo", () => {
    const broken: PilotCatalog = {
      ...
      itineraries: [{ slug: "quebrado", citySlug: "teresina", dayScope: "one-day" as const, title: "Roteiro quebrado", durationLabel: "Teste", summary: "Resumo suficientemente longo para validar.", confirmationNotice: "Aviso suficientemente longo para validar.", stopIds: ["local-inexistente"] }],
    };
