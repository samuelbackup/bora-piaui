...
export type SeedDatabase = NonNullable<
...
function resolveCityIdOrThrow(
  resolver: CityIdResolver,
  citySlug: string
): number {
...
export function buildCuratedBusinessRows(
  resolveCityId: CityIdResolver,
  catalog: PilotCatalog = pilotCatalog
): InsertCuratedBusiness[] {
...
export function buildCurationTopicRows(
  resolveCityId: CityIdResolver,
  catalog: PilotCatalog = pilotCatalog
): InsertCurationTopic[] {
...
export function buildEditorialHighlightRows(
  resolveCityId: CityIdResolver,
  catalog: PilotCatalog = pilotCatalog
): InsertEditorialHighlight[] {
...
export function assertCatalogIntegrity(
  ...
  for (const item of [...catalog.items, ...catalog.curatedBusinesses]) {
    if (!citySlugs.has(item.citySlug))
      problems.push(`Cidade inexistente para "${item.id}": ${item.citySlug}`);
  }
  for (const itinerary of catalog.itineraries) {
    if (!citySlugs.has(itinerary.citySlug))
      problems.push(`Cidade inexistente para o roteiro "${itinerary.slug}"`);
    for (const stopId of itinerary.stopIds) {
      if (!placeIds.has(stopId))
        problems.push(
          `Parada inexistente no roteiro "${itinerary.slug}": ${stopId}`
        );
    }
  }
  for (const relation of catalog.proximityRelations) {
    if (!placeIds.has(relation.anchorItemId))
      problems.push(
        `Âncora inexistente na relação "${relation.id}": ${relation.anchorItemId}`
      );
    if (!placeIds.has(relation.relatedItemId))
      problems.push(
        `Item relacionado inexistente na relação "${relation.id}": ${relation.relatedItemId}`
      );
  }
  for (const business of catalog.curatedBusinesses) {
    for (const anchorId of business.anchorItemIds ?? []) {
      if (!placeIds.has(anchorId))
        problems.push(
          `Âncora inexistente no negócio "${business.id}": ${anchorId}`
        );
    }
  }
  for (const topic of catalog.curationTopics) {
    if (!citySlugs.has(topic.citySlug))
      problems.push(`Cidade inexistente para o tópico "${topic.id}"`);
  }
  for (const highlight of catalog.editorialHighlights) {
    if (!citySlugs.has(highlight.citySlug))
      problems.push(`Cidade inexistente para o destaque "${highlight.id}"`);
  }

  if (problems.length) {
    throw new Error(
      `[seed] Catálogo inconsistente:\n${problems.map(problem => `- ${problem}`).join("\n")}`
    );
  }
}
...
async function upsertCity(db: SeedDatabase, row: InsertCity): Promise<number> {
  ...
  const [saved] = await db
    ...
    .limit(1);
  if (!saved)
    throw new Error(`[seed] Cidade não encontrada após upsert: ${row.slug}`);
  return saved.id;
}
...
async function upsertPlace(
  ...
  const [saved] = await db
    .select({ id: cityPlaces.id })
    .from(cityPlaces)
    .where(eq(cityPlaces.externalId, row.externalId))
    .limit(1);
  if (!saved)
    throw new Error(
      `[seed] Local não encontrado após upsert: ${row.externalId}`
    );
  return saved.id;
}

async function upsertItinerary(
  db: SeedDatabase,
  row: InsertItinerary
): Promise<number> {
  await db
  ...
  const [saved] = await db
    ...
    .from(itineraries)
    .where(eq(itineraries.slug, row.slug))
    .limit(1);
  if (!saved)
    throw new Error(`[seed] Roteiro não encontrado após upsert: ${row.slug}`);
  return saved.id;
}
...
export async function seedPilotCatalog(
  ...
  for (const itinerary of catalog.itineraries) {
    ...
    for (let index = 0; index < itinerary.stopIds.length; index += 1) {
      ...
      if (!placeId)
        throw new Error(`[seed] Parada sem local persistido: ${stopId}`);
      const stop = itineraryStopInsertRowSchema.parse({
        itineraryId,
        placeId,
        sortOrder: index,
      });
  ...
  for (const relation of catalog.proximityRelations) {
    ...
    if (!anchorPlaceId || !relatedPlaceId)
      throw new Error(`[seed] Relação sem locais persistidos: ${relation.id}`);
    ...
    await db
      .insert(placeProximityRelations)
      .values(row)
      .onDuplicateKeyUpdate({
        set: {
...
export async function runSeed() {
  ...
  if (!database) {
    throw new Error(
      "[seed] DATABASE_URL ausente ou banco indisponível. Configure a variável de ambiente e aplique as migrations antes de semear."
    );
  }
...
const invokedDirectly = Boolean(
  process.argv[1]?.replace(/\\/g, "/").endsWith("server/db/seed.ts")
);
if (invokedDirectly) {
  runSeed()
    .then(summary =>
      console.log(`[seed] Concluído: ${JSON.stringify(summary)}`)
    )
    .catch(error => {
      console.error(
        "[seed] Falhou:",
