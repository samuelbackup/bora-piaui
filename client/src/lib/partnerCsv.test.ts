...
describe("buildPartnerProposalsCsv", () => {
  it("cria CSV em formato de planilha brasileira com cabeçalhos em português", () => {
    ...
    expect(
      csv.startsWith(
        "\uFEFFNome do negócio;Cidade;Categoria;Plano;Status editorial;Horário informado;Data de envio\r\n"
      )
    ).toBe(true);
    expect(csv).toContain(
      "Ateliê Caju & Sol;Teresina;Artesanato;destaque;em_revisao"
    );
  ...
  it("escapa aspas, ponto e vírgula e quebras de linha", () => {
    const csv = buildPartnerProposalsCsv([
      { ...proposal, businessName: 'Casa "Raiz"; Piauí\nCentro' },
    ]);
  ...
  it("retorna apenas os cabeçalhos quando não há propostas", () => {
    expect(buildPartnerProposalsCsv([])).toBe(
      "\uFEFFNome do negócio;Cidade;Categoria;Plano;Status editorial;Horário informado;Data de envio\r\n"
    );
  });
});
...
describe("partnerProposalsCsvFilename", () => {
  it("inclui a data na identificação do arquivo", () => {
    expect(
      partnerProposalsCsvFilename(new Date("2026-08-20T12:00:00.000Z"))
    ).toBe("propostas-parceiros-bora-piaui-2026-08-20.csv");
  });
});
