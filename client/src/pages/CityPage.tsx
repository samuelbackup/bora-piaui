...
import { ArrowRight, ChevronLeft, ExternalLink, MapPinned, Phone, Route, Store, TreePine } from "lucide-react";
...
export default function CityPage() {
  ...
  const restaurants = useMemo(() => city && catalog ? getPilotCuratedBusinesses(city.slug, "restaurant", catalog) : [], [catalog, city]);
  const services = useMemo(() => city && catalog ? getPilotCuratedBusinesses(city.slug, "service", catalog) : [], [catalog, city]);
        ...
        <section className="px-4 py-10 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl">
          ...
          <div className="mt-8 grid gap-5 lg:grid-cols-2">
            {items.map((item) => <article key={item.id} className="overflow-hidden rounded-[1.75rem] border border-[#3C482D]/14 bg-[#FFFDF6]">
              ...
              <div className="p-5"><div className="flex flex-wrap items-center justify-between gap-2"><span className="rounded-full bg-[#E9DCC0] px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#566B37]">{item.kind === "attraction" ? "Atração" : "Negócio local"} · {item.category}</span>{item.operationalStatus !== "confirmed" && <span className="text-[11px] font-bold text-[#B9572D]">Operação a confirmar</span>}</div><h3 className="display-font mt-4 text-2xl tracking-[-0.04em]">{item.title}</h3><p className="mt-2 text-sm leading-6 text-[#66705E]">{item.summary}</p>{item.operationalStatus !== "confirmed" && <div className="mt-4"><MvpContentState compact kind="pending" title="Confirmação necessária" description="Horários, condução e demais condições operacionais não são afirmados por este protótipo." /></div>}<div className="mt-4"><SourceLine {...item.source} /></div>{!item.contactUrl && <div className="mt-4"><MvpContentState compact kind="link-unavailable" title="Contato não publicado" description="O canal de contato será exibido somente após confirmação editorial." /></div>}<div className="mt-5 flex flex-wrap gap-2"><Link href={`/cidades/${city.slug}/locais/${item.slug}`} onClick={() => trackMvpEvent("view_item", { item: item.id, city: city.slug })} className="tap inline-flex items-center gap-2 rounded-full bg-[#3C482D] px-4 py-2.5 text-sm font-extrabold text-white hover:bg-[#566B37]">Ver detalhes <ArrowRight className="h-4 w-4" /></Link>{item.routeUrl ? <a href={item.routeUrl} target="_blank" rel="noopener noreferrer" onClick={() => trackMvpEvent("open_route", { item: item.id })} className="tap inline-flex items-center gap-2 rounded-full border border-[#3C482D]/18 px-4 py-2.5 text-sm font-extrabold text-[#3C482D] hover:bg-[#EDE0C4]"><MapPinned className="h-4 w-4 text-[#B9572D]" /> Abrir rota</a> : <span className="inline-flex items-center rounded-full border border-[#3C482D]/10 px-4 py-2.5 text-sm font-bold text-[#7B806F]">Rota indisponível</span>}{item.contactUrl && <a href={item.contactUrl} target="_blank" rel="noopener noreferrer" onClick={() => trackMvpEvent("open_external_link", { item: item.id, source: "contact" })} className="tap inline-flex items-center gap-2 rounded-full border border-[#3C482D]/18 px-4 py-2.5 text-sm font-extrabold text-[#3C482D] hover:bg-[#EDE0C4]"><Phone className="h-4 w-4 text-[#B9572D]" /> Contato</a>}{item.externalUrl && <a href={item.externalUrl} target="_blank" rel="noopener noreferrer" onClick={() => trackMvpEvent("open_external_link", { item: item.id })} className="tap inline-flex items-center gap-2 rounded-full border border-[#3C482D]/18 px-4 py-2.5 text-sm font-extrabold text-[#3C482D] hover:bg-[#EDE0C4]"><ExternalLink className="h-4 w-4" /> Fonte</a>}</div></div>
            </article>)}
          </div>
