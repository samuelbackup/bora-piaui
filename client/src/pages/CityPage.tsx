...
import { ArrowRight, BookOpenText, ChevronLeft, ExternalLink, Landmark, MapPinned, Phone, Route, Store, TreePine, UtensilsCrossed } from "lucide-react";
...
function SourceLine({ name, url, verifiedAt, testId }: { name: string; url: string; verifiedAt: string; testId?: string }) {
  return <p data-testid={testId} className="rounded-xl px-3 py-2 text-xs leading-5 text-[#67705D] transition-colors duration-200 motion-reduce:transition-none group-hover:bg-[#E9DCC0] group-hover:text-[#3C482D] group-focus-within:bg-[#E9DCC0] group-focus-within:text-[#3C482D]">Fonte: <a href={url} target="_blank" rel="noopener noreferrer" className="font-bold text-[#566B37] underline decoration-[#566B37]/35 underline-offset-2">{name}</a> · {verifiedAt}</p>;
}
...
export default function CityPage() {
  ...
  const cityQuery = trpc.cities.getBySlug.useQuery({ slug: citySlug }, { enabled });
  const contentQuery = trpc.cityPlaces.listByCity.useQuery({ citySlug }, { enabled });
  const citiesListQuery = trpc.cities.list.useQuery();
  const itineraryQuery = trpc.itineraries.getByCity.useQuery({ citySlug }, { enabled: enabled && Boolean(cityQuery.data) });

  const city = useMemo(() => (cityQuery.data ? mapCityRow(cityQuery.data) : null), [cityQuery.data]);
  const catalog = useMemo<PilotCatalog | null>(() => {
    ...
    return {
      ...
      curatedBusinesses: content.curatedBusinesses.map(row => mapBusinessRow(row, city.slug)),
      editorialHighlights: content.editorialHighlights.map(row => mapHighlightRow(row, city.slug)),
      itineraries: [],
      proximityRelations: content.proximityRelations.map(mapProximityRelationRow),
      curationTopics: content.curationTopics.map(row => mapTopicRow(row, city.slug)),
    };
  ...
  const foodAnchor = useMemo(() => foodAnchorId && catalog ? getPilotItem(foodAnchorId, catalog) : null, [catalog, foodAnchorId]);
  const foodOptions = useMemo(() => city && catalog && foodAnchor?.citySlug === city.slug && foodAnchor.kind === "attraction" ? getPilotFoodOptions(city.slug, foodAnchor.id, catalog) : [], [catalog, city, foodAnchor]);
  const foodTopic = useMemo(() => curationTopics.find((topic) => topic.category === "gastronomy") ?? null, [curationTopics]);
  const serviceTopics = useMemo(() => curationTopics.filter((topic) => topic.category === "service"), [curationTopics]);

  useEffect(() => {
    if (!city || !catalog) return;
    setCategory("all");
    setActiveMapId(getPilotItems(city.slug, "all", "all", catalog)[0]?.id ?? null);
    const requestedFoodAnchorId = new URLSearchParams(window.location.search).get("para-comer");
    const requestedFoodAnchor = requestedFoodAnchorId ? getPilotItem(requestedFoodAnchorId, catalog) : null;
    setFoodAnchorId(requestedFoodAnchor?.citySlug === city.slug && requestedFoodAnchor.kind === "attraction" ? requestedFoodAnchor.id : null);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      document.getElementById("food-context-title")?.focus({ preventScroll: true });
    }, 0);
  ...
  useEffect(() => {
    if (cityNotFound) trackMvpEvent("ui_error", { scope: "pilot_city", reason: "not_found", slug: params?.slug ?? "unknown" });
  ...
  if (cityNotFound || !city) {
    return <main className="min-h-screen bg-[#F5ECD8] px-4 py-16 text-[#2E3222] sm:px-6"><div className="mx-auto max-w-2xl"><MvpContentState kind="error" title="Cidade fora do recorte atual" description="Este endereço não está entre as cidades-piloto publicadas no atlas." action={<Link href="/" className="tap inline-flex items-center gap-2 rounded-full bg-[#3C482D] px-4 py-2.5 text-sm font-extrabold text-white">Voltar ao atlas <ChevronLeft className="h-4 w-4" /></Link>} /></div></main>;
  }

  if (cityQuery.error || contentQuery.error || !catalog) {
    return <main className="min-h-screen bg-[#F5ECD8] px-4 py-16 text-[#2E3222] sm:px-6"><div className="mx-auto max-w-2xl"><MvpContentState kind="error" title="Não foi possível carregar esta cidade" description="Ocorreu uma falha de conexão com o acervo editorial. Verifique sua internet e tente novamente." action={<button onClick={() => { void cityQuery.refetch(); void contentQuery.refetch(); }} className="tap rounded-full bg-[#3C482D] px-4 py-2.5 text-sm font-extrabold text-white">Tentar novamente</button>} /></div></main>;
  }
      ...
      <main>
        <section className="border-b border-[#3C482D]/10 px-4 py-10 sm:px-6 lg:px-8 lg:py-14" style={{ background: `linear-gradient(135deg, ${city.accent} 0%, #3C482D 72%)` }}>
          <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
            ...
            {itineraryLink && <Link href={`/roteiros/${itineraryLink.slug}`} onClick={() => trackMvpEvent("add_to_itinerary", { city: city.slug, itinerary: itineraryLink.slug })} className="tap inline-flex items-center justify-center gap-2 rounded-full bg-[#FFFDF6] px-5 py-3 text-sm font-extrabold text-[#3C482D] hover:bg-[#F5ECD8]"><Route className="h-4 w-4 text-[#B9572D]" /> Ver roteiro de 1 dia</Link>}
          </div>
        </section>

        <PilotCityNavigator cities={citiesListQuery.data?.map(mapCityRow) ?? []} currentSlug={city.slug} />
        ...
        <section className="px-4 py-10 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl">
          ...
          <div className="mt-8 grid gap-5 lg:grid-cols-2">
            {items.map((item) => <article key={item.id} className="overflow-hidden rounded-[1.75rem] border border-[#3C482D]/14 bg-[#FFFDF6]">
              {item.image ? <figure><img src={item.image.url} alt={item.image.alt} className="h-48 w-full object-cover" />{item.image.credit && <figcaption className="border-b border-[#3C482D]/10 bg-[#F9F0DE] px-5 py-2 text-[10px] font-semibold leading-4 text-[#66705E]">Imagem: {item.image.credit}{item.image.licenseUrl ? <> · <a href={item.image.licenseUrl} target="_blank" rel="noopener noreferrer" className="underline decoration-[#B9572D]/60 underline-offset-2">{item.image.license ?? "licença"}</a></> : item.image.license ? ` · ${item.image.license}` : null}</figcaption>}</figure> : <MvpContentState compact kind="missing-image" title="Imagem em curadoria" description="A interface preserva este espaço até receber uma imagem com crédito e texto alternativo." />}
          ...
          {foodTopic && <section id="para-comer" aria-labelledby="food-context-title" className="mt-8 scroll-mt-6 rounded-[1.75rem] border border-dashed border-[#B9572D]/30 bg-[#FFF8EA] p-5 sm:p-6"><div className="grid gap-5 lg:grid-cols-[1fr_auto] lg:items-end"><div><div className="flex items-center gap-2 text-[#B9572D]"><UtensilsCrossed className="h-4 w-4" /><span className="text-[10px] font-extrabold uppercase tracking-[0.14em]">Para comer</span></div><h3 id="food-context-title" tabIndex={-1} className="display-font mt-3 text-2xl tracking-[-0.04em] outline-none sm:text-3xl">{foodAnchor ? `Opções relacionadas a ${foodAnchor.title}` : foodTopic.title}</h3><p className="mt-2 max-w-2xl text-sm leading-6 text-[#66705E]">{foodAnchor ? "A relação abaixo só mostra negócios com fonte, canal público e confirmação editorial. Não informamos distância ou tempo de deslocamento sem dados verificados." : "Escolha um ponto de partida para preparar a alimentação no percurso. Restaurantes só aparecem depois de curadoria e publicação."}</p></div><label className="block lg:w-[280px]"><span className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-[#68705C]">Ponto de partida</span><select value={foodAnchorId ?? ""} onChange={(event) => { const nextAnchorId = event.target.value || null; setFoodAnchorId(nextAnchorId); if (nextAnchorId) trackMvpEvent("search", { scope: "food_by_anchor", anchor: nextAnchorId, source: "food_selector" }); }} className="mt-2 h-11 w-full rounded-full border border-[#3C482D]/14 bg-[#FFFDF6] px-4 text-sm font-bold outline-none focus:border-[#B9572D] focus:ring-2 focus:ring-[#B9572D]/15"><option value="">Escolha uma atração</option>{getPilotItems(city.slug, "attraction", "all", catalog).map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label></div>{foodAnchor ? foodOptions.length ? <div className="mt-6 grid gap-4 lg:grid-cols-2">{foodOptions.map((business) => <article key={business.id} className="rounded-2xl border border-[#3C482D]/14 bg-[#FFFDF6] p-5"><p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-[#B9572D]">Em curadoria publicada</p><h4 className="display-font mt-3 text-2xl tracking-[-0.04em]">{business.title}</h4><p className="mt-2 text-sm leading-6 text-[#66705E]">{business.summary}</p><div className="mt-4"><SourceLine {...business.source} /></div></article>)}</div> : <div className="mt-6 rounded-2xl border border-[#3C482D]/12 bg-[#FFFDF6] p-5"><p className="font-bold text-[#3C482D]">Opções para comer em curadoria para este ponto.</p><p className="mt-2 text-sm leading-6 text-[#66705E]">Ainda não há estabelecimentos publicados para {foodAnchor.title}. Você pode fazer uma pesquisa externa no mapa e confirmar as informações diretamente com o local.</p><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`restaurantes perto de ${foodAnchor.title}, ${city.name}, PI`)}`} target="_blank" rel="noopener noreferrer" onClick={() => trackMvpEvent("open_external_link", { source: "food_map_search", anchor: foodAnchor.id })} className="tap mt-4 inline-flex items-center gap-2 rounded-full border border-[#3C482D]/18 px-4 py-2.5 text-sm font-extrabold text-[#3C482D] hover:bg-[#EDE0C4]"><MapPinned className="h-4 w-4 text-[#B9572D]" /> Pesquisar no mapa</a></div> : null}</section>}
          {!!serviceTopics.length && <aside aria-label="Serviços em curadoria" className="mt-3 grid gap-3 md:grid-cols-2">{serviceTopics.map((topic) => <div key={topic.id} className="rounded-2xl border border-dashed border-[#3C482D]/20 bg-[#FFFDF6] p-5"><div className="flex items-center gap-2 text-[#B9572D]"><Store className="h-4 w-4" /><span className="text-[10px] font-extrabold uppercase tracking-[0.14em]">Para organizar a visita</span></div><h3 className="display-font mt-3 text-xl tracking-[-0.04em]">{topic.title}</h3><p className="mt-2 text-sm leading-6 text-[#66705E]">{topic.description}</p></div>)}</aside>}
          {editorialHighlights.length > 0 && <section aria-labelledby="cultura-historia-title" data-testid="editorial-highlights" className="mt-10 border-y border-[#3C482D]/12 py-8"><div className="max-w-3xl"><p className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-[#566B37]">Leituras de {city.name}</p><h2 id="cultura-historia-title" className="display-font mt-3 text-3xl tracking-[-0.05em] sm:text-4xl">Cultura e História</h2><p className="mt-3 text-sm leading-6 text-[#66705E]"><strong className="text-[#3C482D]">Curadoria responsável:</strong> conteúdo editorial com base verificável, e não uma lista de recomendações. Cada bloco indica sua fonte pública para consulta.</p></div><div className="mt-7 grid gap-5 lg:grid-cols-2">{editorialHighlights.map((highlight) => <article key={highlight.id} className="group rounded-[1.75rem] border border-[#3C482D]/14 bg-[#FFFDF6] p-6 transition-transform duration-200 hover:-translate-y-0.5 focus-within:-translate-y-0.5 motion-reduce:transform-none motion-reduce:transition-none"><div className="flex items-center gap-3 text-[#B9572D]">{highlight.id.endsWith("cultura") ? <BookOpenText className="h-5 w-5" /> : <Landmark className="h-5 w-5" />}<span className="text-[10px] font-extrabold uppercase tracking-[0.14em]">{city.name} em contexto</span></div><h3 className="display-font mt-4 text-3xl tracking-[-0.05em]">{highlight.title}</h3><p className="mt-3 text-sm leading-6 text-[#66705E]">{highlight.description}</p><div className="mt-5"><SourceLine {...highlight.source} testId={`editorial-source-${highlight.id}`} /></div></article>)}</div></section>}
        </div></section>
        ...
        <section className="bg-[#EDE0C4] px-4 py-12 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-[#566B37]">Mapa sincronizado</p><h2 className="display-font mt-3 text-3xl tracking-[-0.05em] sm:text-4xl">Localize as atrações do recorte.</h2></div><p className="max-w-md text-sm leading-6 text-[#66705E]">O mapa acompanha a lista filtrada. Dados de rota e operação continuam dependentes de confirmação.</p></div>{mapItems.length ? <div className="mt-7"><PiauiMap places={mapItems.map((item) => ({ id: item.id, title: item.title, municipality: city.name, category: item.category, mapQuery: item.mapQuery, accent: item.accent, detailHref: `/cidades/${city.slug}/locais/${item.slug}` }))} activePlaceId={activeMapId} onSelect={(id) => { setActiveMapId(id); trackMvpEvent("view_item", { item: id, source: "city_map" }); }} /></div> : <div className="mt-7"><MvpContentState kind="empty" title="Sem atrações para mapear neste filtro" description="Selecione atrações ou limpe os filtros para disponibilizar os marcadores." /></div>}</div></section>

        <section className="px-4 py-12 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl rounded-[2rem] border border-[#3C482D]/14 bg-[#FFFDF6] p-6 sm:p-8"><div className="grid gap-5 md:grid-cols-[1fr_auto] md:items-center"><div><div className="flex items-center gap-2 text-[#B9572D]"><TreePine className="h-5 w-5" /><span className="text-[10px] font-extrabold uppercase tracking-[0.15em]">Próxima parada</span></div><h2 className="display-font mt-3 text-3xl tracking-[-0.05em]">Planeje com margem para confirmação.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-[#66705E]">{itineraryLink?.confirmationNotice ?? "Os dados de roteiro ainda serão integrados à curadoria."}</p></div>{itineraryLink && <Link href={`/roteiros/${itineraryLink.slug}`} className="tap inline-flex items-center justify-center gap-2 rounded-full bg-[#B9572D] px-5 py-3 text-sm font-extrabold text-white hover:bg-[#CD6D45]"><Store className="h-4 w-4" /> Abrir roteiro</Link>}</div></div></section>
      </main>
    </div>
  );
}
