...
const encontroDosRiosImage = "/manus-storage/encontro-dos-rios-mapacultura_0b698542.jpg";
...
export default function Home() {
  const { data: publishedDestinations, isPending: destinationsPending } = trpc.destinations.list.useQuery();
  const { data: citiesData } = trpc.cities.list.useQuery();
  const { data: itinerariesData } = trpc.itineraries.list.useQuery();
  const pilotCities = useMemo(() => citiesData?.map(mapCityRow) ?? [], [citiesData]);
  const firstPilotCityHref = pilotCities[0] ? `/cidades/${pilotCities[0].slug}` : "/";
  const itineraryByCitySlug = useMemo(() => {
    const slugById = new Map((citiesData ?? []).map(city => [city.id, city.slug]));
