...
describe("acessos territoriais da descoberta", () => {
  ...
  it("prepara uma prévia visual coerente com o polo, o interesse e a busca", () => {
    ...
    expect(
      getPolePreview(previewCatalog, "Costa do Delta", "Litoral", "Barra")?.id
    ).toBe("barra-grande");
    expect(
      getPolePreview(previewCatalog, "Polo Teresina", "Todos", "")?.title
    ).toBe("Encontro dos Rios");
  ...
  it("concentra a navegação em um menu acessível também no desktop", () => {
    ...
    expect(homeSource).not.toContain(
      "hidden items-center gap-5 text-xs font-bold lg:flex"
    );
  });
